...
async def provision_infra(args):
    ...
    print(f"Triggering core infrastructure provisioning for {args.vpc_cidr} in {args.region}...")
...
def cli_parser():
    parser = argparse.ArgumentParser(description="eHub CLI - Engineering Hub Management Tool")
    parser.add_argument("--temporal-host", default="localhost:7233", help="Temporal server address")
    
    subparsers = parser.add_subparsers(dest="command", help="Subcommands")

    # Infra command
    infra_parser = subparsers.add_parser("infra", help="Infrastructure management")
    infra_subparsers = infra_parser.add_subparsers(dest="subcommand", help="Infra subcommands")

    # Infra provision
    provision_parser = infra_subparsers.add_parser("provision", help="Provision core infrastructure")
